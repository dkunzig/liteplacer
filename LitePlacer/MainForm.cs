...
namespace LitePlacer
{
    ...
    public partial class FormMain : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            TapeHeightTextBox.Text = Setting.TapeHeightZ.ToString();
            TapeHeightXTextBox.Text = Setting.TapeHeightX.ToString();
            TapeHeightYTextBox.Text = Setting.TapeHeightY.ToString();

            PCBHeightXTextBox.Text = Setting.PCBHeightX.ToString();
            PCBHeightYTextBox.Text = Setting.PCBHeightY.ToString();
            PCBHeightTextBox.Text = Setting.PCBHeightZ.ToString();
        ...
        private void FormMain_Shown(object sender, EventArgs e)
        {
            ...
            zMaxTravel_textBox.Text = Setting.General_ZtoPCB.ToString("0.00", CultureInfo.InvariantCulture);
        ...
        */
        ...
        private void tabControlPages_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (tabControlPages.SelectedTab.Name)
            {
                ...
                case "Nozzles_tabPage":
                    ...
                    break;
                case "Calibrate_tabPage":
                    ZGuardOn();
                    Calibrate_tabPage_Begin();
                    LastTabPage = "Calibrate_tabPage";
                    break;
            }
        }
        ...
        public void My_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                Movestr = "G1F" + AltJogSpeed_numericUpDown.Value.ToString();
            }
            else if (System.Windows.Forms.Control.ModifierKeys == Keys.Control)
            {
                Movestr = "G1F" + CtlrJogSpeed_numericUpDown.Value.ToString();
            }
            else
            {
                Movestr = "G1F" + NormalJogSpeed_numericUpDown.Value.ToString();
            }
        ...
        private void SetCurrentPosition_button_Click(object sender, EventArgs e)
        {
            ...
            CNC_RawWrite("G28.3X" + Xstr + "Y" + Ystr + "Z" + Zstr + "A" + Astr);
            Thread.Sleep(50);
        }
        ...
        private bool HomingTimeout_m(out int TimeOut, string axis)
        {
            ...
            {
                case "X":
                    speed = homingSeek_maskedTextBox.Text;
        ...
        private void CNC_BlockingWrite_thread(string cmd)
        {
            ...
            DisplayText("blocking write done", KnownColor.DarkRed);
        }

        public bool CNC_Write_m(string s, int Timeout = 5000)
        {
            if (Cnc.ErrorState)
            {
        ...
        private void CNC_BlockingXY_thread(double X, double Y)
        {
            ...
            //Cnc_ReadyEvent.Wait();
            CNC_BlockingWriteDone = true;
            DisplayText("CNC_BlockingXY_thread done", KnownColor.DarkRed);
        }
        ...
        private void CNC_BlockingXYA_thread(double X, double Y, double A)
        {
            Cnc_ReadyEvent.Reset();
            Cnc.XYA(X, Y, A);
            //Cnc.XYA(X, Y, A); //put this back
            //Cnc_ReadyEvent.Wait();
            CNC_BlockingWriteDone = true;
            DisplayText("CNC_BlockingXYA_thread done", KnownColor.DarkRed);
        }
        ...
        private void CNC_BlockingZ_thread(double Z)
        {
            ...
            //Cnc_ReadyEvent.Wait();
            CNC_BlockingWriteDone = true;
            DisplayText("CNC_BlockingZ_thread done", KnownColor.DarkRed);
        }
        ...
        private void CNC_BlockingA_thread(double A)
        {
            ...
            Cnc_ReadyEvent.Wait();
            CNC_BlockingWriteDone = true;
            DisplayText("CNC_BlockingA_thread done", KnownColor.DarkRed);
        }
        ...
        private void BasicSetupTab_Begin()
        {
            ...
            UpdateCncConnectionStatus();
            xMaxTravel_textBox.Text = Setting.General_MachineSizeX.ToString();
            yMaxTravel_textBox.Text = Setting.General_MachineSizeY.ToString();
            ...
            //RefreshPortList();
            if (websocketiptextBox.Equals(string.Empty))
            {
                return;
            };
        }
        ...
        {
            if (websocketiptextBox.Text.Equals(string.Empty))
            {
                return;  // no ip
            };
            ...
            if (Cnc.ErrorState || !Cnc.Connected)
            {
                ...
                if (Cnc.Connect(websocketiptextBox.Text))
                {
                    buttonConnectSerial.Text = "Connecting..";
                    Cnc.ErrorState = false;
                    Setting.CNC_SerialPort = websocketiptextBox.Text;
                    UpdateCncConnectionStatus();
                    CNC_Write_m("$$");
                }
            }
        ...
        */
        ...
        public void ValueUpdater(List<GRBLSettings> settings)
        {
            if (InvokeRequired) { Invoke(new Action<List<GRBLSettings>>(ValueUpdater), new[] { settings }); return; }
            // DisplayText("ValueUpdater: item= " + item + ", value= " + value);
            foreach (GRBLSettings sets in settings)
            {
                DisplayText("setting= " + sets.setting + " value =" + sets.value);
                string setting = sets.setting;
                ...
                switch (setting)
                {
                    ...
                    case "$100":
                        Update_1tr(value);
                        break;
                    case "$110":    // x velocity maximum, mm/min
                        Update_xvm(value);
                        break;
                    case "$120":    // x feedrate maximum, mm/mi
                        Update_xjm(value);
                        break;
                    case "130":        // x travel minimum, mm
                        xMaxTravel_textBox.Text = value;
                        break;
                    case "$24":        // x travel minimum, mm
                        homingFeed_maskedTextBox.Text = Convert.ToDouble(value).ToString();
                        break;
                    case "$25":        // x travel minimum, mm
                        homingSeek_maskedTextBox.Text = Convert.ToDouble(value).ToString();
                        break;
                    case "$27":        // x travel minimum, mm
                        homingPullOff_maskedTextBox.Text = Convert.ToDouble(value).ToString();
                        break;
                    ...
                    case "$101":
                        Update_2tr(value);
                        break;
                    case "$111":    // x velocity maximum, mm/min
                        Update_yvm(value);
                        break;
                    case "$121":    // x feedrate maximum, mm/mi
                        Update_yjm(value);
                        break;
                    case "131":        // x travel minimum, mm
                        yMaxTravel_textBox.Text = value;
                        break;
                    ...
                    case "$102":
                        Update_3tr(value);
                        break;
                    case "$112":    // x velocity maximum, mm/min
                        Update_zvm(value);
                        break;
                    case "$122":    // x feedrate maximum, mm/mi
                        Update_zjm(value);
                        break;
                    case "132":        // x travel minimum, mm
                        zMaxTravel_textBox.Text = value;
                        break;
                    // ========== A axis ==========
                    case "$103":
                        Update_4tr(value);
                        break;
                    case "$113":    // x velocity maximum, mm/min
                        Update_avm(value);
                        break;
                    case "$123":    // x feedrate maximum, mm/mi
                        Update_ajm(value);
                        break;
...
#endif
            xAccel_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_yjm(string value)
        {
            ...
            double val = Convert.ToDouble(value) / 1000000;
#endif
            yAccel_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_zjm(string value)
        {
            ...
            double val = Convert.ToDouble(value) / 1000000;
#endif
            zAccel_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_ajm(string value)
        {
            ...
            double val = Convert.ToDouble(value) / 1000000;
#endif
            aAccel_maskedTextBox.Text = val.ToString();
        }
        ...
        {
            xAccel_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$120=" + xAccel_maskedTextBox.Text);
                Thread.Sleep(50);
                xAccel_maskedTextBox.ForeColor = Color.Black;
            }
        }

        private void yjm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            yAccel_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$121=" + yAccel_maskedTextBox.Text);
                Thread.Sleep(50);
                yAccel_maskedTextBox.ForeColor = Color.Black;
            }
        }

        private void zjm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            zAccel_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$122=" + zAccel_maskedTextBox.Text);
                Thread.Sleep(50);
                zAccel_maskedTextBox.ForeColor = Color.Black;
            }
        }

        private void ajm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            aAccel_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$123=" + zAccel_maskedTextBox.Text);
                Thread.Sleep(50);
                aAccel_maskedTextBox.ForeColor = Color.Black;
            }
        }
        ...
        private void Update_xjh(string value)
        {
            ...
            double val = Convert.ToDouble(value) / 1000000;
#endif
            homingFeed_maskedTextBox.Text = val.ToString();
        }

        // =========================================================================
        // *jh setting

        private void homingfeed_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            homingFeed_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$24 = " + homingFeed_maskedTextBox.Text);
                Thread.Sleep(50);
                homingFeed_maskedTextBox.ForeColor = Color.Black;
            }
        }

        private void homingseek_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            homingSeek_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$25 = " + homingSeek_maskedTextBox.Text);
                Thread.Sleep(50);
                homingSeek_maskedTextBox.ForeColor = Color.Black;
            }
        }
        private void homingpulloff_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            homingPullOff_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$27 = " + homingPullOff_maskedTextBox.Text);
                Thread.Sleep(50);
                homingPullOff_maskedTextBox.ForeColor = Color.Black;
            }
        }
        ...
        private void Update_xsv(string value)
        {
            ...
            double val = Convert.ToDouble(value);
            homingSeek_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_xvm(string value)
        {
            ...
            double val = Convert.ToDouble(value);
            xMaxRate_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_yvm(string value)
        {
            ...
            double val = Convert.ToDouble(value);
            yMaxRate_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_zvm(string value)
        {
            ...
            double val = Convert.ToDouble(value);
            zMaxRate_maskedTextBox.Text = val.ToString();
        }
        ...
        private void Update_avm(string value)
        {
            ...
            double val = Convert.ToDouble(value);
            aMaxRate_maskedTextBox.Text = val.ToString();
        }
        ...
        {
            xMaxRate_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$110=" + xMaxRate_maskedTextBox.Text);
                Thread.Sleep(50);
                xMaxRate_maskedTextBox.ForeColor = Color.Black;
            }
        }

        private void yvm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            yMaxRate_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$111=" + yMaxRate_maskedTextBox.Text);
                Thread.Sleep(50);
                yMaxRate_maskedTextBox.ForeColor = Color.Black;
            }
        }

        private void zvm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            zMaxRate_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$112=" + yMaxRate_maskedTextBox.Text);
                Thread.Sleep(50);
                zMaxRate_maskedTextBox.ForeColor = Color.Black;
                int peek = Convert.ToInt32(zMaxRate_maskedTextBox.Text);
        ...
        {
            aMaxRate_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("$113=" + yMaxRate_maskedTextBox.Text);
                Thread.Sleep(50);
                aMaxRate_maskedTextBox.ForeColor = Color.Black;
                int peek = Convert.ToInt32(aMaxRate_maskedTextBox.Text);
        ...
        {
            xStepsPer_mm_textBox.Text = value;
        }

        private void Update_2tr(string value)
        {
            yStepsPer_mm_textBox.Text = value;
        }

        private void Update_3tr(string value)
        {
            zStepsPer_mm_textBox.Text = value;

        }

        private void Update_4tr(string value)
        {
            aStepsPer_mm_textBox.Text = value;
        }
        ...
        {
            double val;
            xStepsPer_mm_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(xStepsPer_mm_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$100 = " + xStepsPer_mm_textBox.Text);
                    Thread.Sleep(50);
                    xStepsPer_mm_textBox.ForeColor = Color.Black;
                }
            }
        }
        ...
        {
            double val;
            yStepsPer_mm_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(yStepsPer_mm_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$101 = " + yStepsPer_mm_textBox.Text);
                    Thread.Sleep(50);
                    yStepsPer_mm_textBox.ForeColor = Color.Black;
                }
            }
        }
        ...
        {
            double val;
            zStepsPer_mm_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(zStepsPer_mm_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$102 = " + zStepsPer_mm_textBox.Text);
                    Thread.Sleep(50);
                    zStepsPer_mm_textBox.ForeColor = Color.Black;
                }
            }
        }
        ...
        {
            double val;
            aStepsPer_mm_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(aStepsPer_mm_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$103 = " + aStepsPer_mm_textBox.Text);
                    Thread.Sleep(50);
                    aStepsPer_mm_textBox.ForeColor = Color.Black;
                }
            }
        }
        ...
        // * update
        public void Update_xpos(string value)
        {
        ...
        public void Update_ypos(string value)
        {
        ...
        public void Update_zpos(string value)
        {
            if (InvokeRequired) { Invoke(new Action<string>(Update_zpos), new[] { value }); return; }
            //xpos_textBox.Text = Cnc.CurrentZ.ToString("0.000", CultureInfo.InvariantCulture);
            zpos_textBox.Text = Cnc.CurrentZ.ToString("0.000", CultureInfo.InvariantCulture);
        }

        public void Update_apos(string value)
        {
            if (InvokeRequired) { Invoke(new Action<string>(Update_apos), new[] { value }); return; }
            apos_textBox.Text = Cnc.CurrentA.ToString("0.000", CultureInfo.InvariantCulture);
        }
        ...
        {
            xMaxTravel_textBox.ForeColor = Color.Red;
            double val;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(xMaxTravel_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$130 = " + val);
                    Setting.General_MachineSizeX = val;
                    xMaxTravel_textBox.ForeColor = Color.Black;
                    DownCamera.SideMarksX = Setting.General_MachineSizeX / 100;
                }
            }
        }
        ...
        {
            double val;
            yMaxTravel_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(yMaxTravel_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$131 = " + val);
                    Setting.General_MachineSizeY = val;
                    yMaxTravel_textBox.ForeColor = Color.Black;
                    DownCamera.SideMarksY = Setting.General_MachineSizeY / 100;
                }
            }
        }
        ...
        private void zMaxTravel_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            zMaxTravel_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(zMaxTravel_textBox.Text.Replace(',', '.'), out val))
                {
                    CNC_Write_m("$132 = " + val);
                    Setting.General_ZtoPCB = val;
                    zMaxTravel_textBox.ForeColor = Color.Black;
                }
            }
        }
        ...
        private void CalibrateYDownButton_Click(object sender, EventArgs e)
        {
            double y;
            if (double.TryParse(CalibrateYDistanceTextBox.Text, out y))
                CNC_XY_m(Cnc.CurrentX, Cnc.CurrentY - y);
            else
                CNC_XY_m(Cnc.CurrentX, Cnc.CurrentY - .1);
        }

        private void CalibrateYUpButton_Click(object sender, EventArgs e)
        {
            double y;
            if (double.TryParse(CalibrateYDistanceTextBox.Text, out y))
                CNC_XY_m(Cnc.CurrentX, Cnc.CurrentY + y);
        ...
        private void CalibrateXRightButton_Click(object sender, EventArgs e)
        {
            double x;
            if (double.TryParse(CalibrateXDistanceTextBox.Text, out x))
                CNC_XY_m(Cnc.CurrentX + x, Cnc.CurrentY);
            else
                CNC_XY_m(Cnc.CurrentX + .1, Cnc.CurrentY);
        }

        private void CalibrateXLeftButton_Click(object sender, EventArgs e)
        {
            double x;
            if (double.TryParse(CalibrateXDistanceTextBox.Text, out x))
                CNC_XY_m(Cnc.CurrentX - x, Cnc.CurrentY);
        ...
        private void CalibrateZUpButton_Click(object sender, EventArgs e)
        {
            double z;
            if (double.TryParse(CalibrateZDistanceTextBox.Text, out z))
                CNC_Z_m(Cnc.CurrentZ - z);
            else
                CNC_Z_m(Cnc.CurrentZ - 1);
        }

        private void CalibrateZDownButton_Click(object sender, EventArgs e)
        {
            double z;
            if (double.TryParse(CalibrateZDistanceTextBox.Text, out z))
                CNC_Z_m(Cnc.CurrentZ + z);
        ...
        private void RotateALeftButton_Click(object sender, EventArgs e)
        {
            double A;
            if (double.TryParse(MotorANumberOfSteps.Text, out A))
                CNC_A_m(Cnc.CurrentA - A);
            else
                CNC_A_m(Cnc.CurrentA - .1);
        }

        private void RotateARightButton_Click(object sender, EventArgs e)
        {
            double A;
            if (double.TryParse(MotorANumberOfSteps.Text, out A))
                CNC_A_m(Cnc.CurrentA + A);
        ...
        {
            General_pictureBox_MouseClick(Calibrate_pictureBox, e.X, e.Y);
        }
        ...
        private void FixDelimiter(ref string[] AllLines)
        {
            int i, j;
            AllLines[0] = FixHeader(AllLines[0]);
            for (i = 0; i < AllLines.Length; i++)
            {
                AllLines[i] = AllLines[i].TrimStart();
                if (AllLines[i] == null)
                    continue;
                StringBuilder stringBuilder = new StringBuilder(AllLines[i].Length);
                for (j = 0; j < AllLines[i].Length; j++)
                {
                    char c = AllLines[i][j];
                    if (j == 0 || c != ' ' || (c == ' ' && AllLines[i][j - 1] != ' '))
                        stringBuilder.Append(c);
                }
                AllLines[i] = stringBuilder.ToString().Replace(' ', ',');
            }
        ...
        {
            header = header.Replace("Mid X", "MidX");
            header = header.Replace("Mid Y", "MidY");
            header = header.Replace("Ref X", "RefX");
            header = header.Replace("Ref Y", "RefY");
            header = header.Replace("Pad X", "PadX");
            header = header.Replace("Pad Y", "PadY");
            return header;
        }
        private bool ParseCadData_m(String[] AllLines, bool KiCad)
        {
            ...
            List<string> XList = new List<string> { "x", "x (mm)", "Center-X(mm)", "PosX", "midx" };
            for (i = 0; i < Headers.Count; i++)
            {
            ...
            List<string> YList = new List<string> { "y", "y (mm)", "Center-y(mm)", "Posy", "midy" };
            for (i = 0; i < Headers.Count; i++)
            {
            ...
            foreach (var Line in DataLines)
            {
                ...
                if (Line.Count < 7)
                    continue;
                double x;
                if (!double.TryParse(Line[X_Nominal_Index].ToString().Replace(',', '.').Replace("mm", ""), out x))
                    continue;
                if (!double.TryParse(Line[Y_Nominal_Index].ToString().Replace(',', '.').Replace("mm", ""), out x))
                    continue;
                if (LayerDataPresent)
                {
        ...
        private void Calibrate_tabPage_Begin()
        {
            SetDownCameraDefaults();
            SelectCamera(DownCamera);
            DownCamera.ImageBox = CalibratePictureBox;
        ...
        private void Calibrate_tabPage_End()
        {
            ZGuardOn();
        }
        ...
        public void SaveConfigSettings()
        {
            ...
            AppSettingsSave_button_Click(this, ea);
        }
...
*/
        ...
        {
            CNC_RawWrite("G28.3A0");
        }

        private void webSocket1_received_data(object sender, MessageEventArgs e)
        {
            //DisplayText("WebSocket message = " + e.Message, KnownColor.Blue, true);
            Cnc.InterpretLine(e.Data);
        }

        private void Placement_pictureBox_Click(object sender, EventArgs e)
        {

        }

        private void PlaceNozzelButton_Click(object sender, EventArgs e)
        {
            double HoleX, HoleY;
            string Z_str = Tapes_dataGridView.Rows[0].Cells["Z_Pickup_Column"].Value.ToString();
            CNC_Z_m(0);
            if (!Tapes.GotoNextPartByMeasurement_m(0, out HoleX, out HoleY))
            {
                ShowMessageBox(
                   "Can't find tape hole",
                   "Tape error",
            ...
            CNC_Z_m(Convert.ToDouble(Z_str));
        }

        private void PositionNozzelYPlus_Click(object sender, EventArgs e)
        {
            double Y;
            double offsetY = Convert.ToDouble(NozzleOffsetY_textBox.Text);
            if (!double.TryParse(NozzelOffsetMoveY.Text, out Y))
                Y = 1;
            NozzleOffsetY_textBox.Text = ((offsetY + Y).ToString());
            Setting.DownCam_NozzleOffsetY = offsetY + Y;
        }

        private void PositionNozzelYMinus_Click(object sender, EventArgs e)
        {
            double Y;
            double offsetY = Convert.ToDouble(NozzleOffsetY_textBox.Text);
            if (!double.TryParse(NozzelOffsetMoveY.Text, out Y))
        ...
        private void PositionNozzelXMinus_Click(object sender, EventArgs e)
        {
            double X;
            double offsetX = Convert.ToDouble(NozzleOffsetX_textBox.Text);
            if (!double.TryParse(NozzelOffsetMoveX.Text, out X))
                X = 1;
            NozzleOffsetX_textBox.Text = ((offsetX - X).ToString());
            Setting.DownCam_NozzleOffsetX = offsetX - X;
        }

        private void PositionNozzelXPlus_Click(object sender, EventArgs e)
        {
            double X;
            double offsetX = Convert.ToDouble(NozzleOffsetX_textBox.Text);
            if (!double.TryParse(NozzelOffsetMoveX.Text, out X))
        ...
        private void PlaceNozzelAtCameraPositionButton_Click(object sender, EventArgs e)
        {
            //double dX, dY;
            //if (!Nozzle.CorrectedPosition_m(Cnc.CurrentA, out dX, out dY))
            //{
            //    MessageBox.Show("Nozzel Has Not Been calibrated.", "Calibrate Nozzel", MessageBoxButtons.OK);
            //    return;
            //};

            //Cnc.XY_move(Cnc.CurrentX + settings.DownCam_NozzleOffsetX + dX, Cnc.CurrentY + settings.DownCam_NozzleOffsetY + dY);
            //while (Cnc.ZMotorBusy || Cnc.YMotorBusy) { Application.DoEvents(); }
            //Cnc.Z(Convert.ToDouble(settings.PCBHeightZ));
        }



        private void PositionNozzelXPlus_Click_1(object sender, EventArgs e)
        {

        }

        private void SetTapeHeightButton_Click(object sender, EventArgs e)
        {
            TapeHeightTextBox.Text = Cnc.CurrentZ.ToString();
            Setting.TapeHeightZ = Cnc.CurrentZ;

            TapeHeightYTextBox.Text = Cnc.CurrentY.ToString();
            Setting.TapeHeightY = Cnc.CurrentY;

            TapeHeightXTextBox.Text = Cnc.CurrentX.ToString();
            Setting.TapeHeightX = Cnc.CurrentX;

            SaveConfigSettings();
            
        }

        private void PCBHeightButton_Click(object sender, EventArgs e)
        {
            PCBHeightTextBox.Text = Cnc.CurrentZ.ToString();
            Setting.PCBHeightZ = Cnc.CurrentZ;

            PCBHeightXTextBox.Text = Cnc.CurrentX.ToString();
            Setting.PCBHeightX = Cnc.CurrentX;

            PCBHeightYTextBox.Text = Cnc.CurrentY.ToString();
            Setting.PCBHeightY = Cnc.CurrentY;
        ...
        {
            double X, Y, Z;
            if (double.TryParse(PCBHeightTextBox.Text, out Z) && double.TryParse(PCBHeightYTextBox.Text, out Y) && double.TryParse(PCBHeightXTextBox.Text, out X))
            {
            ...
            {
                ShowMessageBox(
                                    "The X, Y and Z coordinates must contain values",
                                    "Cant Move Without X,Y and Z Values",
                                    MessageBoxButtons.OK);
                return;
            }
            CNC_Z_m(Z);
        ...
        private void TapeHeightTextBox_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            double X, Y, Z;
            if (double.TryParse(TapeHeightTextBox.Text, out Z) && double.TryParse(TapeHeightYTextBox.Text, out Y) && double.TryParse(TapeHeightXTextBox.Text, out X))
            {
        ...
        private void SetHeightButton_Click(object sender, EventArgs e)
        {
            ...
            {
                row.HeaderCell.Value = row.Index.ToString();
                row.Cells[15].Value = TapeHeightTextBox.Text;
                row.Cells[16].Value = PCBHeightTextBox.Text;
            }
        }

        private void CalibrateZGuardOffCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (NozzleZGuard_checkBox.Checked)
            {
                ZGuardOff();
        ...
        private void PlaceNozzelAtCameraPositionButton_Click_1(object sender, EventArgs e)
        {
            CNC_XY_m(Cnc.CurrentX + Setting.DownCam_NozzleOffsetX, Cnc.CurrentY + Setting.DownCam_NozzleOffsetY);
        }
        ...
        private void websocketiptextBox_TextChanged(object sender, EventArgs e)
        {
        ...
        private void labelSerialPortStatus_Click(object sender, EventArgs e)
        {
